public class Joueur {
    ...
    public int joue(Plateau p, MEE s, int[] nbPointsJet) {
        ...
        if (choice == 'P')
            ...
            Utils.clear();
            System.out.print(p + "\n\n" + this);
            System.out.println("\u001B[31m" + "\u001B[1m" + "\n\nErreur: " + "\u001B[38;5;208m" + input + Utils.RESET
    ...
    public boolean joueMot(Plateau p, MEE s, int[] nbPointsJet) {
        ...
        try {
            mot = Utils.inputString();
            System.out.print("\u001B[0m");
        } catch (Exception ex) {
            ex.printStackTrace();
        }

        System.out.print("\n\nVeuillez saisir la position du mot (Format: " + Utils.BLUE + "D-9" + Utils.RESET + ")\n> "
        + Utils.GREEN);
        String pos = Utils.inputString();
        System.out.print(Utils.RESET);
        String[] position = pos.split("-");

        int x = Utils.majToIndex(position[0].charAt(0)) + 1;
        int y = Integer.parseInt(position[1]);

        char sens = ' ';
        System.out.print("\nVeuillez saisir le sens du mot (h/v)\n> " + "\u001B[32m");
        try {
            sens = Utils.inputChar();
            System.out.print("\u001B[0m");
        } catch (Exception ex) {
            ex.printStackTrace();
        ...
        while (sens != 'h' && sens != 'v') {
            System.out.println(
                    "\u001B[31m" + "\u001B[1m" + "\nErreur: " + "\u001B[0m"
                            ...
                            + "\u001B[34m" + "h" + Utils.RESET + " ou " + Utils.BLUE + "v" + Utils.RESET + ".");
            System.out.print("\nVeuillez saisir le sens du mot (h/v)\n> " + "\u001B[32m");
            try {
                sens = Utils.inputChar();
                System.out.print("\u001B[0m");
        ...
        System.out
                .print("\u001B[31m" + "\n\nM. CapeloDico" + "\u001B[0m" + ", acceptez-vous le mot " + "\u001B[34m"
                        ...
                        + "\u001B[0m" + " (o/n) ? \n> " + "\u001B[32m");
        try {
            rep = Character.toUpperCase(Utils.inputChar());
            System.out.print("\u001B[0m");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void echangeJetons(MEE sac) {
        ...
        try {
            selectedTokens = Utils.inputString();
            System.out.print("\u001B[0m");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public boolean estCorrectPourEchange(String mot) {
        ...
        while (i < mot.length() && isCorrect) {
            if (!Utils.estUneMajuscule(mot.charAt(i)))
                isCorrect = false;
            else if (this.chevalet.getFrequence(Utils.majToIndex(mot.charAt(i))) == 0)
                isCorrect = false;
            else
                i++;
        }
    ...
    public void echangeJetonsAux(MEE sac, String ensJetons) {
        ...
        for (int i = 0; i < ensJetons.length(); i++) {
            this.chevalet.transfere(sac, Utils.majToIndex(ensJetons.charAt(i)));
        }
    }
}
